...
public interface AIJobRepo extends JpaRepository<AIJob, Long> {
    ...
    List<AIJob> findAllByUserIdAndPromptContainingIgnoreCaseOrderByCreatedAtDesc(
            Long userId,
            String keyword
    );

    List<AIJob> findAllByUserIdAndTaskTypeInOrderByCreatedAtDesc(
            Long userId,
            List<String> taskTypes
    );

    List<AIJob> findAllByUserIdAndStatusAndResultFileNameIsNotNullOrderByCreatedAtDesc(
            Long userId,
            JobStatus status
    );
}